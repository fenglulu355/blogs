    ...
    <div class="warpper">
      <van-swipe :autoplay="3000" touchable indicator-color="#82C41C">
        <van-swipe-item v-for="(item, index) in slides" :key="index">
          <img @click="todetail(item)" v-lazy="baseurl+`/public/`+item.image_url" :key="index" alt />
          <!-- <p>{{index}}</p> -->
        </van-swipe-item>
      </van-swipe>
    </div>
    ...
    requst() {
      ...
      this.$axios.post("/index/api/getBanner").then(res => {
        for (let index = 0; index < res.data.data.length; index++) {
          if (res.data.data[index].banner_type_id == 3) {
    ...
    requstKind(val, page) {
      this.$axios
        ...
        .then(res => {
          ...
          // this.slides = res.data.data.data.slice(4, 7);
        });
    },
    getList(page) {
