...
export default {
  ...
  methods: {
    tocase() {
      this.$router.push({ path: "/cases", query: { id: this.class_id } });
      sessionStorage.setItem("curindex", 2);
      let leftindex = sessionStorage.getItem("caselfindex");
      sessionStorage.setItem("caseproClass", leftindex);
      // 跳转回到初始位置
    ...
    toNav(item, index) {
      ...
      this.proClass = index;
      // 存储产品分类下标
      sessionStorage.setItem("caselfClass", index);
